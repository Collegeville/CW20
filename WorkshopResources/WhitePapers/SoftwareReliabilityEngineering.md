...
operate continuously and efficaciously. SRE has gained widespread adoption
throughout the commercial software services industry.
...
While *Customer Support* is focused primarily on *responding to questions*, SRE is
focused on the continous investment of effort and evolution of best practices to
craft and carry out *small corrective actions* aimed at continously improving the
...
and aspects of [User Experience Driven Development (UXDD)][UXDD].


https://medium.com/@carlosjoel.tavares_25618/ux-driven-development-9a35cee2c3b1


Typically, [SRE]+[UXDD] issues originate from users in the midst of using a software product
and are encountering some kind of difficulty. The work involved in handling
...
For mature DOE software projects with wide reach and many users, [SRE]+[UXDD] activity
...
no dedicated [SRE]+[UXDD] resources. Instead, developers themselves must also support SRE
activity. Nonetheless, managing [SRE]+[UXDD] work effectively and efficiently is an
...
Some of the goals of [SRE]+[UXDD] are...
...
* To *load balance* [SRE]+[UXDD] work in an equitable way across the development team.
* To reduce [SRE]+[UXDD] interruptions for the team as a whole.
* To log, track and evolve a database of [SRE]+[UXDD] activity and effort to help inform
  ongoing development plans and resource allocation.
* To identify and document escalation paths for major incidents.

While many aspects of [SRE]+[UXDD] are under the direct control of software developers,
some (those involving issues in the underlying computing infrastructure) are
...
[SRE]+[UXDD] work is allocated and rotated among developers in *shifts*. During
...
[escalations](#escalation), any other developers are free of [SRE]+[UXDD]
...
within the response time goal, to each inquiry. Ideally, all [SRE]+[UXDD] activity during
...
wind up engaging other developers. In addition, any active [SRE]+[UXDD] issues that
remain unresolved at the end of a shift are formally handed off to the next
shift's **Primary**.

Active [SRE]+[UXDD] issues are logged and tracked in an issue tracker separate from
the project's planned development. Upon resolution of serious incidents, the
...
Because [SRE]+[UXDD] work tends to be [interrupt driven](#), there is always the chance
...
particular, there is no expectation that a developer serving as [SRE]+[UXDD] **Primary**
can get any other work done beyond their active or idle [SRE]+[UXDD] obligations. In slow
...
The preceding paragraphs describe [SRE]+[UXDD] processes at a
...
different category of work, [SRE]+[UXDD], that is essential part of maintaining
...
**Primary** handles as part of their [SRE]+[UXDD] activity. The comprehensive solution,
...
[SRE]+[UXDD] activities and developing a practice of continuously crafting constructive
corrective actions arising from [SRE]+[UXDD] inquiries.

Consequently, another key role of the **Primary** is to use any time not working
active [SRE]+[UXDD] issues to fix other *low-hanging fruit* issues from the
...
Active [SRE]+[UXDD] issues are logged and tracked in a separate issue tracking system apart
from product development.
For each new inquiry, the **Primary** files an issue ticket and assign themselves.
When the inquiry is resolved, the associated
issue is closed. The primary will endeavor to capture all relevant information and
communications in this issue.

For any work the **Primary** performs, even if it is a rather trivial amount
of work to resolve, there should be an associated [SRE]+[UXDD] issue for tracking that work.
Tracking even the trivial issues will help to build a database 
to later mine to identify where further product or process improvements can be made.

## Supported Platforms for First Contact

An [SRE]+[UXDD] inquiry with the team begins with a *first contact* and may optionally
...
involve are the responsibility of the [SRE]+[UXDD] **Primary**. Consequently, spontaneous
first contacts can wind up jeopardizing the goals of [SRE]+[UXDD] processes by making it
difficult to track, allocate and manage [SRE]+[UXDD] effort.
...
effort to hand-off the inquiry to the **Primary** [SRE]+[UXDD] as quickly and politely as
practical.
...
It is important to distinguish between *response* and *resolution* of [SRE]+[UXDD] incidents.
...
*Resolution* of an [SRE]+[UXDD] issue often involves one or more of the
following activities...
  ...
  * Identifying any *low-hanging fruit* type work that would address, even if
    only in part, the original [SRE]+[UXDD] inquiry and then engaging in the
    work to resolve it.
...
issue can be addressed to *satisfaction* within the constraints of the [SRE]+[UXDD]
...
The [SRE]+[UXDD] issue gets *promoted* to a product development issue. It is closed
in the [SRE]+[UXDD] issue tracker and new issue is opened in the product development
issue tracker including a reference to the original [SRE]+[UXDD] issue. Doing so does
serve to *resolve* the original [SRE]+[UXDD] issue that initiated the work.
...
When such incidents reach [SRE]+[UXDD] resolution, the **Primary** prepares a
...
These [SRE]+[UXDD] processes involve two kinds of *handoffs*. One is the
redirection of a customer who makes first contact with a developer not serving as
the **Primary**. The other is the handoff of unresolved [SRE]+[UXDD] issues
...
process, such as reducing [SRE]+[UXDD] interruptions for the team as a whole and tracking
[SRE]+[UXDD] effort.
